...
export const createFee = async (input) => {
    ...
    if (existingFee) {
        throw new AppError("A fee with this code already exists", 409);
    }
    try {
...
export const updateFee = async (id, input) => {
    const existingFee = await prisma.fee.findUnique({
        where: {
            id,
        },
    ...
    if (input.code !== undefined) {
        ...
        if (normalizedCode !==
            existingFee.code) {
            const duplicate = await prisma.fee.findUnique({
                where: {
                    code: normalizedCode,
                },
                select: {
...
export const deleteFee = async (id) => {
    ...
    if (fee._count.invoiceItems > 0) {
        throw new AppError("This fee cannot be deleted because it is already used by invoice items", 409, [
            {
                invoiceItems: fee._count.invoiceItems,
            },
        ]);
    }
    try {
