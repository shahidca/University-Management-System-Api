...
import { resultPublishedNotification, } from "../notifications/notification.templates.js";
...
export const publishResult = async (resultId) => {
    ...
    validateAcademicState({
        sectionActive: result.exam.section.isActive,
        offeringActive: result.exam.section
    ...
    if (result.enrollment.status !==
        "ENROLLED") {
        throw new AppError("Result cannot be published for an inactive enrollment", 400);
    }
    /*
     ...
     */
    const publication = await prisma.result.updateMany({
        where: {
            id: resultId,
            status: "APPROVED",
        },
    ...
    if (publication.count !== 1) {
        throw new AppError("Result could not be published because its status has changed", 409);
    }
    /*
    ...
    if (!publishedResult) {
        throw new AppError("Published result could not be retrieved", 500);
    }
    /*
    ...
    if (!student) {
         ...
         */
        console.error("Student profile not found while creating result publication notification:", result.enrollment.studentId);
     ...
     */
    const notification = resultPublishedNotification(result.exam.section
        .courseOffering.course.code, result.exam.title);
