...
export const refreshUserToken = async (input) => {
    let payload;
    try {
    ...
    const tokenMatches = hashToken(input.refreshToken) === storedToken.tokenHash;
    if (!tokenMatches) {
...
export const verifyUserEmail = async (input) => {
    const user = await prisma.user.findUnique({
        where: {
            email: input.email,
        },
    ...
    if (user.emailVerifiedAt) {
        throw new AppError("Email address is already verified", 409);
    }
    ...
    const otpMatches = providedOtpHash === otpRecord.codeHash;
    if (!otpMatches) {
        const updatedOtp = await prisma.oTP.updateMany({
            where: {
                id: otpRecord.id,
                attempts: {
                    lt: MAX_OTP_ATTEMPTS,
...
export const resendVerificationOtp = async (input) => {
    const user = await prisma.user.findUnique({
        where: {
            email: input.email,
        },
...
export const forgotPassword = async (input) => {
    ...
    if (latestOtp) {
        ...
        const elapsedMs = now.getTime() -
            latestOtp.createdAt.getTime();
        if (elapsedMs < cooldownMs) {
            throw new AppError("Please wait before requesting another password reset code", 429);
        }
    }
    const otp = generateOtp();
    const otpHash = hashOtp(otp);
...
export const resetPassword = async (input) => {
    ...
    if (!otpRecord) {
        throw new AppError("Password reset code not found. Please request a new code", 400);
    }
    if (otpRecord.expiresAt <= new Date()) {
...
export const googleLogin = async (idToken) => {
    ...
    if (existingGoogleUser) {
        ...
        if (existingGoogleUser.role !== "STUDENT") {
            throw new AppError("Google login is available only for student accounts", 403);
        }
        return createGoogleAuthSession(existingGoogleUser.id, existingGoogleUser.role, existingGoogleUser.email, existingGoogleUser.firstName, existingGoogleUser.lastName, existingGoogleUser.status);
    }
    /*
    ...
    if (existingEmailUser) {
        ...
        const linkedUser = await prisma.user.update({
            ...
            data: {
                googleId: googleUser.googleId,
                emailVerifiedAt: existingEmailUser.emailVerifiedAt ??
                    new Date(),
            },
        });
        return createGoogleAuthSession(linkedUser.id, linkedUser.role, linkedUser.email, linkedUser.firstName, linkedUser.lastName, linkedUser.status);
    }
    /*
    ...
    return createGoogleAuthSession(newUser.id, newUser.role, newUser.email, newUser.firstName, newUser.lastName, newUser.status);
};
export const createGoogleAuthSession = async (userId, role, email, firstName, lastName, status) => {
    const tokenId = randomUUID();
    const accessToken = generateAccessToken({
        userId,
        role,
