...
export const createProgram = async (input) => {
    ...
    if (existingProgram) {
        throw new AppError("A program with this name already exists in the department", 409);
    }
    ...
    if (existingCode) {
        throw new AppError(existingCode.deletedAt
            ? "Program code already exists on a deleted program"
            : "Program code already exists", 409);
    }
    const program = await prisma.program.create({
        data: {
            ...
            totalCredits: new Prisma.Decimal(input.totalCredits),
...
export const getPrograms = async (query) => {
    const { page, limit, search, departmentId, degree, isActive, sortBy, sortOrder, } = query;
...
export const updateProgram = async (id, input) => {
    const existingProgram = await prisma.program.findFirst({
        where: {
            id,
            deletedAt: null,
    ...
    if (input.code !== undefined &&
        ...
        const duplicateCode = await prisma.program.findUnique({
            where: {
                code: input.code,
            },
        });
...
export const deleteProgram = async (id) => {
    ...
    if (program._count.students > 0) {
        throw new AppError("Cannot delete a program with enrolled students", 409);
    }
