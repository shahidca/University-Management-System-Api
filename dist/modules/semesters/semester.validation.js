...
export const createSemesterSchema = z
    ...
    code: z
        ...
        .max(30, "Semester code must not exceed 30 characters")
        .toUpperCase(),
    ...
    registrationOpen: z
        .string()
        .datetime({
        message: "Invalid registration open date",
    }),
    registrationClose: z
        .string()
        .datetime({
    ...
    const registrationOpen = new Date(data.registrationOpen);
    const registrationClose = new Date(data.registrationClose);
    ...
    if (registrationClose > startDate) {
        ctx.addIssue({
            code: "custom",
            message: "Registration must close on or before the semester start date",
            path: ["registrationClose"],
        });
    }
});
