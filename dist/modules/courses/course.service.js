...
export const createCourse = async (input) => {
    ...
    if (existingCourse) {
        throw new AppError(existingCourse.deletedAt
            ? "Course code belongs to a deleted course"
            : "Course code already exists", 409);
    }
...
export const getCourses = async (query) => {
    const { page, limit, departmentId, courseType, level, isActive, search, sortBy, sortOrder, } = query;
...
export const activateCourse = async (id) => {
    ...
    if (course.isActive) {
        throw new AppError("Course is already active", 409);
    }
...
export const deactivateCourse = async (id) => {
    ...
    if (!course.isActive) {
        throw new AppError("Course is already inactive", 409);
    }
    return prisma.course.update({
        where: {
            id,
...
export const deleteCourse = async (id) => {
    ...
    if (course._count.offerings > 0) {
        throw new AppError("Cannot delete a course that has course offerings", 409);
    }
