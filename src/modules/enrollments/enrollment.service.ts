...
const getStudentByUserId = async (
  ...
  if (
    !student.program.isActive ||
    student.program.deletedAt
  ) {
    throw new AppError(
      "Student's academic program is inactive",
      400,
    );
  }
...
const getSectionForEnrollment = async (
  sectionId: string,
) => {
  const section =
  ...
  if (
    ...
    throw new AppError(
      "Course registration is not open for this semester",
      400,
    );
  }
  ...
  if (
    now < semester.registrationOpen ||
    now > semester.registrationClose
  ) {
    throw new AppError(
      "Course registration is outside the allowed registration window",
      400,
    );
  }
...
const validatePrerequisites = async (
  ...
  for (const prerequisite of prerequisites) {
    ...
    if (!previousEnrollment) {
      throw new AppError(
        `Prerequisite course ${prerequisite.prerequisite.code} - ${prerequisite.prerequisite.title} has not been completed`,
        400,
      );
    }

    const finalResult =
      previousEnrollment.results[0];

    if (!finalResult) {
      throw new AppError(
        `No published final result found for prerequisite course ${prerequisite.prerequisite.code}`,
        400,
      );
    }
    ...
    if (actualGrade === "F") {
      throw new AppError(
        `Prerequisite course ${prerequisite.prerequisite.code} was not passed`,
        400,
      );
    }
    ...
    if (requiredPoint === undefined) {
      throw new AppError(
        `Invalid minimum grade configuration for prerequisite ${prerequisite.prerequisite.code}`,
        500,
      );
    }
    ...
    if (
      ...
      throw new AppError(
        `Student does not meet the minimum grade requirement for prerequisite course ${prerequisite.prerequisite.code}`,
        400,
        [
          {
            prerequisiteCourse:
...
const validateFinancialEligibility = async (
  studentId: string,
) => {
  ...
  throw new AppError(
    "Course registration is blocked because you have an overdue invoice",
    403,
    [
      {
...
const validateSemesterCreditLimit = async (
  studentId: string,
  semesterId: string,
  newCourseCredits: Prisma.Decimal,
) => {
  ...
  if (
    ...
    throw new AppError(
      `Semester credit limit exceeded. Maximum allowed credits are ${MAX_SEMESTER_CREDITS}`,
      400,
      [
        {
          currentCredits,
...
const validateScheduleConflicts = async (
  ...
  const targetSchedules =
    await prisma.sectionSchedule.findMany({
      where: {
        sectionId,
      },
  ...
  for (const existingEnrollment of existingEnrollments) {
    for (const existingSchedule of existingEnrollment
      ...
      for (const targetSchedule of targetSchedules) {
        ...
        throw new AppError(
          "Schedule conflict detected with an already enrolled course",
          409,
          [
            {
              existingCourse:
...
export const createEnrollment = async (
  ...
  const section =
    await getSectionForEnrollment(
      input.sectionId,
    );

  const semesterId =
    section.courseOffering.semester.id;

  const courseId =
    section.courseOffering.course.id;
  ...
  if (existingEnrollment) {
    throw new AppError(
      `Student already has an enrollment for this section with status ${existingEnrollment.status}`,
      409,
    );
  }
   ...
   */
  await validateFinancialEligibility(
    student.id,
  );
   ...
   * Semester credit limit.
   */
  await validateSemesterCreditLimit(
    student.id,
    semesterId,
    courseCredits,
  );
...
export const getEnrollments = async (
  query: EnrollmentListQueryInput,
) => {
  const {
    page,
    limit,
...
export const dropEnrollment = async (
  ...
  if (
    ...
    throw new AppError(
      "You do not have permission to modify this enrollment",
      403,
    );
  }
  ...
  if (
    ...
    throw new AppError(
      `Enrollment cannot be dropped from its current status: ${enrollment.status}`,
      400,
    );
  }
  ...
  if (
    ...
    throw new AppError(
      "Enrollment cannot be dropped after the semester is completed",
      400,
    );
  }
