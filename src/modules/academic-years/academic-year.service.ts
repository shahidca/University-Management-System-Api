...
const checkDateOverlap = async (
  ...
  if (overlappingYear) {
    throw new AppError(
      `Academic year overlaps with "${overlappingYear.name}"`,
      409,
    );
  }
};
...
export const getAcademicYears = async (
  query: AcademicYearListQueryInput,
) => {
  const {
    page,
    limit,
...
export const deactivateAcademicYear =
  async (id: string) => {
    const academicYear =
      await prisma.academicYear.findUnique({
        where: {
    ...
    if (semesters > 0) {
      throw new AppError(
        "Cannot deactivate an academic year with an active semester",
        409,
      );
    }
