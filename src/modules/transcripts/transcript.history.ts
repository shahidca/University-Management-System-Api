...
export const getStudentAcademicHistory =
  async (userId: string) => {
    ...
    for (const result of results) {
      ...
      const courseResult: GpaCourseResult = {
        courseId: course.id,
    ...
    const cumulativeCourses: GpaCourseResult[] =
      [];
        ...
        ({ semester, courses }) => {
          ...
          return {
            ...
            semesterQualityPoints:
              semesterCalculation.totalQualityPoints,

            cumulativeGpa:
              cumulativeCalculation.gpa,

            cumulativeCredits:
              cumulativeCalculation.totalCredits,

            cumulativeQualityPoints:
              cumulativeCalculation.totalQualityPoints,
          };
        },
      );

    const overallCalculation =
      calculateCgpa(cumulativeCourses);
    ...
    return {
      ...
      summary: {
        ...
        totalCredits:
          overallCalculation.totalCredits,

        totalQualityPoints:
          overallCalculation.totalQualityPoints,

        cgpa:
          overallCalculation.gpa,
      },
